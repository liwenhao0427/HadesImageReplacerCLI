...
在中间的替换效果图上可以拖拽生成裁剪区域。裁剪区域按原始游戏资源宽高比生成：拖拽只指定高度和位置，宽度会自动跟随比例变化；裁剪框允许超出替换图本身，保存时超出的区域会变成透明像素，便于给主体周围补留白。存在裁剪框时，方向键按“角色移动方向”调整位置，点按步进较小，长按会逐步加速且松开即停；`+` / `-` 会放大或缩小裁剪框。也可以在高度输入框里手动输入最终替换效果图的裁剪高度像素，或通过 X/Y 输入框和滑条快速控制裁剪框中心位置。右侧叠图会实时显示保存后再按生成 Mod 规则缩放的最终效果。点击“保存当前裁剪”后会用裁剪后的图片覆盖当前替换 PNG，并把第一次保存前的原图备份到替换目录旁的 `_hir_preview_backup` 目录。备份已存在时不会再次覆盖。
